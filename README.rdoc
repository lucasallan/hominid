...
You do not need to pass your api_key to each method, as that is done automatically for you:
  ...
  h.lists({}, page_number, limit_by_page)
  # Get all lists with pagination in the same order that they appear in the mailchimp
  h.lists({}, page_number, limit_by_page, 'web', 'ASC')
